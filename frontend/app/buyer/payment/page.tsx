...
function PaymentInner() {
  ...
  const handlePlaceOrder = async () => {
    ...
    setLoading(true);
    try {
      // Step 1 — Create the order in our backend (status: PENDING)
      ...
      // Step 2 — Store amount in sessionStorage for the callback page to use
      sessionStorage.setItem(`order_amount_${newOrderId}`, String(grandTotal));

      // Step 3 — Initialize Interswitch payment via our API route
      const payRes = await fetch("/api/payment/initialize", {
        ...
        body: JSON.stringify({
          orderId: newOrderId,
          amount: grandTotal,
          customerEmail: "buyer@farm.ng",
          customerName: "PrismAgro Buyer",
          description: `${listing.cropType} × ${qty}kg from ${listing.farmerName}`,
        }),
      });
      ...
      if (payData.paymentUrl) {
        // Step 4 — Redirect buyer to Interswitch checkout page
        window.location.href = payData.paymentUrl;
      } else {
        // Fallback for sandbox/dev when IPG URL isn't available
        setStep("success");
      }
    } catch {
      // Dev fallback — show success without real payment
    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-32 rounded-2xl bg-[#E6FEE7] animate-pulse"
  ...
  if (step === "success")
    return (
      <div className="max-w-md mx-auto py-16 text-center">
        <div
          className="w-20 h-20 rounded-full bg-[#E6FEE7] flex items-center justify-center mx-auto mb-6"
        ...
        <h1
          className="text-2xl font-black text-[#023103] mb-2"
          style={{ fontFamily: "var(--font-display)" }}
        >
          Order Placed! 🎉
        </h1>
        <p className="text-sm text-[#06930A] mb-2">
          Order ID: <span className="font-bold text-[#023103]">{orderId}</span>
        </p>
        <p className="text-sm text-gray-500 mb-8 max-w-xs mx-auto">
          <strong className="text-[#023103]">
            ₦{grandTotal.toLocaleString()}
          </strong>{" "}
          is now held securely in{" "}
          <strong className="text-[#023103]">Prism Vault</strong>. Released only
          when you confirm delivery.
        </p>
        <div className="bg-white rounded-2xl border border-[#E6FEE7] p-5 mb-6 text-left">
          <p className="text-xs font-bold text-[#046207] mb-4 uppercase tracking-wide">
            What happens next
          </p>
          {[
            {
              icon: ShieldCheck,
              label: "Payment held in Prism Vault",
              done: true,
              color: "#08C40E",
            },
            {
              icon: Leaf,
              label: `${listing?.farmerName} prepares your order`,
              done: false,
              color: "#046207",
            },
            {
              icon: Truck,
              label: "Logistics driver picks up & delivers",
              done: false,
              color: "#3B82F6",
            },
            {
              icon: CheckCircle,
              label: "You confirm delivery — funds released to farmer",
              done: false,
              color: "#8B5CF6",
            },
          ].map((s, i) => (
            ...
            >
              <div
                ...
                style={{
                  background: s.done ? s.color : "#F5FFF5",
                  border: `1.5px solid ${s.done ? s.color : "#CEFDCF"}`,
                }}
              >
                <s.icon
                  size={14}
                  style={{ color: s.done ? "white" : s.color }}
                />
              </div>
              <p
                className={`text-sm ${s.done ? "font-semibold text-[#023103]" : "text-gray-400"}`}
              >
                {s.label}
              </p>
            </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <h1
        className="text-2xl font-black text-[#023103] mb-6"
        style={{ fontFamily: "var(--font-display)" }}
      >
        Checkout
      </h1>
      ...
      {listing && (
        <div className="bg-white rounded-2xl border border-[#E6FEE7] p-5 mb-4">
          <div className="flex items-center gap-4 mb-3">
            <div className="w-16 h-16 rounded-xl bg-linear-to-br from-[#E6FEE7] to-[#CEFDCF] flex items-center justify-center text-3xl shrink-0">
              {CROP_EMOJI[listing.cropType] ?? "🌱"}
            </div>
            <div className="flex-1 min-w-0">
              <h2
                className="font-black text-[#023103] text-lg"
                style={{ fontFamily: "var(--font-display)" }}
              >
                {listing.cropType}
              </h2>
            ...
            <div className="text-right shrink-0">
              <p
                className="text-xl font-black text-[#08C40E]"
                style={{ fontFamily: "var(--font-display)" }}
              >
          ...
          {listing.description && (
            <p className="text-xs text-gray-500 bg-[#F5FFF5] rounded-xl p-3">
              {listing.description}
            </p>
          )}
        </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E6FEE7] p-5 mb-4">
        <label className="block text-xs font-bold text-[#046207] uppercase tracking-wide mb-3">
          Quantity (kg)
        </label>
        <div className="flex items-center gap-4">
          ...
          <button
            onClick={() =>
              setQty((q) => Math.min(listing?.quantity ?? 999, q + 1))
            }
            className="w-10 h-10 rounded-xl border border-[#CEFDCF] text-[#046207] font-bold text-xl hover:bg-[#F0FEF1] transition-colors flex items-center justify-center"
          >
            +
      ...
      <div className="bg-white rounded-2xl border border-[#E6FEE7] p-5 mb-4">
        ...
        <div className="relative">
          <MapPin
            size={16}
            className="absolute left-3 top-3 text-[#06930A]"
          />
          <textarea
      ...
      <div className="bg-white rounded-2xl border border-[#E6FEE7] p-5 mb-6">
        <p className="text-xs font-bold text-[#046207] uppercase tracking-wide mb-4">
          Order Summary
        </p>
        <div className="space-y-2 text-sm">
          <div className="flex justify-between text-gray-500">
            <span>
              {listing?.cropType} × {qty}kg
            </span>
            <span>₦{total.toLocaleString()}</span>
          </div>
          <div className="flex justify-between text-gray-500">
            <span>Logistics fee (8%)</span>
        ...
        <div className="mt-4 flex items-start gap-2 p-3 rounded-xl bg-[#F0FEF1] border border-[#CEFDCF]">
          <ShieldCheck
            size={16}
            className="text-[#08C40E] shrink-0 mt-0.5"
          />
          <p className="text-xs text-[#046207]">
            <strong>Prism Vault Protection:</strong> Funds held in escrow until
            you confirm delivery. 100% refundable if there's an issue.
          </p>
        </div>
      </div>
      ...
      <button
        ...
        style={{
          ...
          boxShadow: "0 6px 24px rgba(8,196,14,0.3)",
        }}
      >
        {loading ? (
          <>
      ...
      <p className="text-center text-xs text-gray-400 mt-3">
        By placing this order you agree to the PrismAgro escrow terms.
      </p>
    </div>
  );
}
